    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-xs sm:text-sm text-slate-500 mt-1">Detailed leave reports and analytics</p>
            </div>
    ...
    <div class="py-6 sm:py-8 bg-[#F8FAFC]" x-data="{ mounted: false }" x-init="mounted = true">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8"
                ...
                <!-- Total Requests -->
                <a href="{{ route('admin.leave-summary.index', array_merge(request()->query(), ['status' => 'all'])) }}" 
                   class="bg-white/80 backdrop-blur-sm border border-[#E5E7EB] shadow-[0_1px_3px_0_rgba(0,0,0,0.05)] rounded-[20px] p-4 sm:p-6 transition-all duration-300 hover:shadow-[0_4px_12px_0_rgba(0,0,0,0.08)] hover:-translate-y-1 cursor-pointer block {{ $status === 'all' ? 'ring-2 ring-indigo-500 border-indigo-500' : '' }}">
                ...
                <!-- Approved -->
                <a href="{{ route('admin.leave-summary.index', array_merge(request()->query(), ['status' => 'approved'])) }}" 
                   class="bg-white/80 backdrop-blur-sm border border-[#E5E7EB] shadow-[0_1px_3px_0_rgba(0,0,0,0.05)] rounded-[20px] p-4 sm:p-6 transition-all duration-300 hover:shadow-[0_4px_12px_0_rgba(0,0,0,0.08)] hover:-translate-y-1 cursor-pointer block {{ $status === 'approved' ? 'ring-2 ring-emerald-500 border-emerald-500' : '' }}">
                    <div class="flex items-center gap-3 sm:gap-4">
                ...
                <!-- Pending -->
                <a href="{{ route('admin.leave-summary.index', array_merge(request()->query(), ['status' => 'pending'])) }}" 
                   class="bg-white/80 backdrop-blur-sm border border-[#E5E7EB] shadow-[0_1px_3px_0_rgba(0,0,0,0.05)] rounded-[20px] p-4 sm:p-6 transition-all duration-300 hover:shadow-[0_4px_12px_0_rgba(0,0,0,0.08)] hover:-translate-y-1 cursor-pointer block {{ $status === 'pending' ? 'ring-2 ring-amber-500 border-amber-500' : '' }}">
                    <div class="flex items-center gap-3 sm:gap-4">
                ...
                <!-- Rejected -->
                <a href="{{ route('admin.leave-summary.index', array_merge(request()->query(), ['status' => 'rejected'])) }}" 
                   class="bg-white/80 backdrop-blur-sm border border-[#E5E7EB] shadow-[0_1px_3px_0_rgba(0,0,0,0.05)] rounded-[20px] p-4 sm:p-6 transition-all duration-300 hover:shadow-[0_4px_12px_0_rgba(0,0,0,0.08)] hover:-translate-y-1 cursor-pointer block {{ $status === 'rejected' ? 'ring-2 ring-rose-500 border-rose-500' : '' }}">
                    <div class="flex items-center gap-3 sm:gap-4">
            ...
            <div class="bg-white/80 backdrop-blur-sm border border-[#E5E7EB] shadow-[0_1px_3px_0_rgba(0,0,0,0.05)] rounded-[20px] p-4 sm:p-6 mb-6 sm:mb-8"
                 ...
                 x-transition:enter-start="opacity-0 translate-y-4"
                 x-transition:enter-end="opacity-100 translate-y-0">
                <form method="GET" action="{{ route('admin.leave-summary.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <!-- Status Filter -->
                    <div>
                    ...
                    <!-- Division Filter -->
                    <div>
                        <label for="division" class="block text-xs font-semibold text-[#6B7280] mb-2">Division</label>
                        <select name="division" id="division" class="w-full px-4 py-2.5 border border-[#E5E7EB] rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                            <option value="all" {{ $division === 'all' ? 'selected' : '' }}>All Divisions</option>
                            @foreach($divisions as $div)
                                <option value="{{ $div->id }}" {{ $division == $div->id ? 'selected' : '' }}>{{ $div->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="bg-white/80 backdrop-blur-sm border border-[#E5E7EB] shadow-[0_1px_3px_0_rgba(0,0,0,0.05)] rounded-[20px] p-4 sm:p-6 transition-all duration-300"
                ...
                <div class="flex items-center justify-between mb-4 sm:mb-6">
                    <div>
                        <h3 class="text-base sm:text-lg font-semibold text-slate-900 mb-1">Leave Requests</h3>
                        <p class="text-xs sm:text-sm text-[#6B7280]">Filtered leave requests list</p>
                    </div>
                </div>
