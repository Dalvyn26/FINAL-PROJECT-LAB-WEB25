                    ...
                    <div class="md:hidden space-y-4">
                        @foreach($leaveRequests as $index => $request)
                            <div class="bg-white border border-[#E5E7EB] rounded-2xl p-4 shadow-sm hover:shadow-md transition-all duration-300"
                                ...
                                @if($request->isRejected() && $request->rejection_note)
                                    <div class="mb-3 p-2 bg-rose-50 rounded-lg">
                                        <p class="text-xs font-semibold text-rose-900 mb-1">Catatan Penolakan:</p>
                                        <p class="text-xs text-rose-700">{{ Str::limit($request->rejection_note, 100) }}</p>
                                    </div>
                                @endif
                                <div class="flex items-center gap-2 pt-3 border-t border-[#E5E7EB]">
                                    ...
                                    @if($request->status === 'approved')
                                        <a href="{{ route('leave-requests.download-pdf', $request) }}" 
                                           class="inline-flex items-center gap-2 text-rose-600 hover:text-rose-700 font-medium text-sm transition-colors hover:underline"
                                           title="Download Surat Cuti (PDF)">
